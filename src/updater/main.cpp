...
int applyUpdate(const QStringList &arguments)
{
    ...
    if (archivePath.isEmpty() || installDirectory.isEmpty() || !QFileInfo::exists(archivePath)) {
        QMessageBox::critical(nullptr, QStringLiteral("Atlas Studio Updater"),
...
int checkForUpdate(QApplication &application, const QStringList &arguments)
{
    ...
    QObject::connect(&service, &atlas::UpdateService::updateAvailable, &application,
        ...
        prompt.setWindowTitle(QStringLiteral("Доступно обновление Atlas Studio"));
        prompt.setText(QStringLiteral("Доступна версия %1.").arg(release.version));
        prompt.setInformativeText(QStringLiteral("Обновление будет скачано с GitHub Releases и проверено по SHA-256. После согласия Atlas Studio закроется, файлы заменятся, затем IDE запустится снова."));
        ...
        if (prompt.clickedButton() == update) {
            ...
            progress->setWindowTitle(QStringLiteral("Atlas Studio Updater"));
    ...
    QObject::connect(&service, &atlas::UpdateService::updateDownloadError, &application,
        ...
        QMessageBox::critical(nullptr, QStringLiteral("Atlas Studio Updater"), message);
        application.quit();
    });
    QObject::connect(&service, &atlas::UpdateService::updateReadyToInstall, &application,
                     [&application, installDirectory, parentPid](const QString &archivePath, const atlas::UpdateRelease &) {
        QString error;
        const QString restartPath = QDir(installDirectory).filePath(QStringLiteral("AtlasStudio.exe"));
        if (!launchTemporaryUpdater(archivePath, installDirectory, restartPath, parentPid, &error)) {
            QMessageBox::critical(nullptr, QStringLiteral("Atlas Studio Updater"), error);
        }
        application.quit();
    });
...
int main(int argc, char *argv[])
{
    ...
    application.setApplicationName(QStringLiteral("Atlas Studio Updater"));
    const QStringList arguments = application.arguments();
    if (arguments.contains(QStringLiteral("--help")) || arguments.contains(QStringLiteral("-h"))) {
        QTextStream(stdout) << "AtlasUpdater --check --repo owner/repository --current-version x.y.z "
                               "--install-dir path --settings-dir path --parent-pid pid\\n"
                               "AtlasUpdater --apply --archive file.zip --install-dir path "
                               "--restart AtlasStudio.exe --target-pid pid\\n";
        return 0;
    }
